...
1. **Khối mã chuẩn Markdown:** Bắt buộc dùng khối mã ` ```mermaid ` (Hugo Goldmark tự động biên dịch thành `<div class="mermaid" id="...">` qua Render Hook `themes/LoveIt/layouts/_markup/render-codeblock-mermaid.html`).
2. **Khởi tạo và Token màu:** Đã được cấu hình tự động qua hàm `initMermaid()` trong `assets/js/theme.js` (chứa `theme: 'base'`, `securityLevel: 'loose'`, `padding: 32`, `themeVariables`). Hệ thống tự động đồng bộ thời điểm render sau khi `document.fonts.ready` hoàn tất để đo đạc bounding box chính xác 100% theo glyph của font chữ chính. **TUYỆT ĐỐI KHÔNG** chèn thủ công `%%{init}%%` vào bài viết.
3. **Dark Mode và Render Timing:** Hệ thống sử dụng bộ lọc CSS GPU Filter trên container `.mermaid svg` để đổi màu tức thì 0ms khi chuyển theme sáng/tối, không re-render lại SVG làm giật lag giao diện.
4. **Chuẩn Font và Hiển Thị Tự Nhiên (Natural Scale):** Mermaid đồng bộ sử dụng font serif thanh lịch của dự án (`--font-serif` / Newsreader) giúp chữ hiển thị tự nhiên, trang nhã và đồng bộ với nội dung bài viết. Khung node được cấu hình độ đệm ngang rộng rãi (`padding: 32`, `wrappingWidth: 320`) giúp hộp chữ nhật dài hơn, tạo không gian thoáng đãng và loại bỏ hoàn toàn tình trạng chữ chạm hoặc tràn mép viền. Hệ thống tắt phóng đại toàn màn hình (`useMaxWidth: false` và `width: auto; max-width: 100%`) để các sơ đồ nhỏ không bị kéo dãn bự quá mức, trong khi các sơ đồ lớn vẫn co dãn mượt mà trên thiết bị di động. Khoảng cách giữa các từ luôn giữ ở mức chuẩn tự nhiên (`word-spacing: normal !important; letter-spacing: normal !important;`), tuyệt đối không thêm khoảng trống (space) phụ giữa các từ. **TUYỆT ĐỐI KHÔNG** hardcode các font khác (như `system-ui`, `Roboto`, `Arial`) hoặc can thiệp sai lệch kích thước chữ làm lệch bounding box tính toán của Mermaid JS.
